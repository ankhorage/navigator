...
describe('@ankhorage/navigator generation safety', () => {
  test('validates module bindings without flow-specific generation fields', () => {
    const plan = createNavigatorPlan(
      { type: 'stack', routes: [{ name: 'sign-in', screenId: 'home' }] },
      { platform: 'ios', expoRouterVersion: '56.0.0' },
    );
    expect(() =>
  ...
  test('blocks generation when semantic validation fails', () => {
    ...
    expect(generateNavigatorFiles(plan, { screens: SCREEN_BINDINGS, guards: {} })).toMatchObject({
      support: 'unsupported',
      files: [],
    });
  });
...
describe('@ankhorage/navigator tabs planning', () => {
  ...
  test('resolves registered and responsive headless presentations deterministically', () => {
    expect(
      resolveHeadlessTabsPresentation(
